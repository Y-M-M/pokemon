...
UDPPokemon** Login::getPokemon()//只读，返回3个Pokemon
{
    return pokemon;
}
...
void Login:: readPendingDatagrams()//读取数据报
{
    ...
    QDataStream dsIn(&data, QIODevice:: ReadWrite);//创建QDataStream对象 dsIn，用来对data进行读写操作
    ...
    if(loginKind == SIGNINOK)//如果收到的消息是SIGNOK
    {
        ...
        for(int i = 0; i < 3; i++)//读取服务端发送来的三个小精灵
        {
            UDPPokemon *udppokemon = new UDPPokemon;
            dsIn>>*udppokemon;
            pokemon[i] = udppokemon;
            qDebug() << "接收到一个小精灵";
        }
