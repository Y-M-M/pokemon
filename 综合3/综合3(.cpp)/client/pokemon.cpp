...
QDataStream &operator<<(QDataStream &stream, const UDPPokemon &the_pokemon)
{
...
QDataStream &operator>>(QDataStream &stream, UDPPokemon &the_pokemon)
{
    stream >> the_pokemon.name;
    stream >> the_pokemon.kind;
    stream >> the_pokemon.level;
...
BASE_Pokemon::BASE_Pokemon(const UDPPokemon& udppokemon)
{
    grade = udppokemon.level;
    max_EXP = udppokemon.max_EXP;
    EXP = udppokemon.EXP;
    normal_ATK = udppokemon.ATK;
    DF = udppokemon.DF;
    HP = udppokemon.HP;
    max_HP = udppokemon.max_HP;
    speed = udppokemon.speed;
}
...
TYPE_Pokemon:: TYPE_Pokemon(const UDPPokemon& udppokemon): BASE_Pokemon(udppokemon)
{
    kind = udppokemon.kind;
}
...
//构造函数，通过UDPPokemon构造Pokemon
Pokemon:: Pokemon(const UDPPokemon& udppokemon): TYPE_Pokemon(udppokemon)
{
    name = udppokemon.name.toStdString();
    special_ATK = udppokemon.special_ATK;
    special_ATK_name = udppokemon.special_ATK_name.toStdString();
}
...
void Pokemon:: update(int* before_attr)//升级、更新参数
{
    before_attr[0] = check_grade();
    before_attr[1] = check_HP();
    before_attr[2] = check_ATK();
    before_attr[3] = check_DF();
    before_attr[4] = check_speed();
    before_attr[5] = check_EXP();
    addgrade();
    switch(check_kind())
    {
    case 0:
