...
public:
    ...
    int attack_each_other(int* before_attr);//在统一时间线下相互攻击
    int battle(int* before_attr);//控制战斗的开始，输出信息、将血量加满
    int judge_end(int& iswin, int* before_attr) const;//判断战斗是否结束，增加惊艳、升级
private:
