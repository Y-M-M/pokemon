...
public:
    BASE_Pokemon();//构造函数
    BASE_Pokemon(const UDPPokemon& udppokemon);//由udppokemon构造pokemon
...
public:
    ...
    TYPE_Pokemon(const UDPPokemon& udppokemon);//由udppokemon构造普通pokemon
...
public:
    ...
    Pokemon(const UDPPokemon& udppokemon);//从udppokemon转换为普通pokemon
    ~Pokemon();//析构函数
    ...
    void update(int* before_attr);//升级、更新参数
